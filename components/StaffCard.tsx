...
const StaffCard: React.FC<StaffCardProps> = ({ member, viewMode, onEdit }) => {
  ...
  const getProviderIcon = (provider: string) => {
    ...
    if (p.includes("microsoft") || p.includes("azure"))
      return "fa-brands fa-microsoft text-blue-400";
    ...
    if (p.includes("meta") || p.includes("react"))
    ...
    if (p.includes("cncf") || p.includes("kubernetes"))
      return "fa-solid fa-dharmachakra text-blue-500";
  ...
  return (
    ...
    >
        ...
        <div className="relative">
          <div className="w-32 h-32 rounded-2xl overflow-hidden border-4 border-white dark:border-slate-800 shadow-lg ring-2 ring-sky-400/30">
            {!imgError ? (
              ...
              <div
                className={`w-full h-full flex items-center justify-center text-3xl font-black text-white ${getRoleColor(member.role)}`}
              >
          ...
          <div
            className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full border-4 border-white dark:border-slate-800 ${getRoleColor(member.role)} shadow-md flex items-center justify-center text-[10px] text-white font-bold`}
          >
      ...
      <div className="text-center mb-6 w-full">
        <h3 className="text-xl font-black truncate px-2 dark:text-white">
          {member.name}
        </h3>
          ...
          <div className="space-y-3">
            ...
            {DAYS.map((day) => {
              ...
              return (
                <div
                  key={day}
                  className={`flex items-center justify-between p-3 rounded-2xl transition-colors ${loc ? "bg-sky-50 dark:bg-sky-900/20 border border-sky-100 dark:border-sky-900/30" : "opacity-30"}`}
                >
                  ...
                  {loc ? (
                    ...
                    <span className="text-[9px] italic font-bold dark:text-slate-400">
                      Base (TT)
                    </span>
                  )}
                </div>
