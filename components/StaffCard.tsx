...
const StaffCard: React.FC<StaffCardProps> = ({ member, viewMode, onEdit }) => {
  ...
  const handleCopyAsImage = async () => {
    ...
    try {
      ...
      // Masquer les boutons d'action complètement pour la capture
      const actionButtons = cardRef.current.querySelectorAll('.action-button');
      ...
      actionButtons.forEach(btn => (btn as HTMLElement).style.display = 'flex');
    } catch (err) {
