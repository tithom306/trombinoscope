    ...
    return projects
      .map((project) => ({
        ...
        members: project.members.filter((member) => {
          ...
          const matchesSkill = member.skills.some((s) =>
    ...
    <div className="min-h-screen transition-colors duration-500 font-sans bg-sky-50 dark:bg-slate-950">
        ...
        <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex bg-gray-100 dark:bg-slate-800 p-1 rounded-xl">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === "talents" ? "bg-white dark:bg-slate-700 text-sky-600" : "text-gray-400"}`}
            >
              Personnel
            </button>
            <button
              onClick={() => setViewMode("planning")}
              className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === "planning" ? "bg-white dark:bg-slate-700 text-sky-600" : "text-gray-400"}`}
            >
              Plan de Vol
            </button>
            <button
              onClick={() => setViewMode("offices")}
              className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === "offices" ? "bg-white dark:bg-slate-700 text-sky-600" : "text-gray-400"}`}
            >
              Appareils
            </button>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
          ...
          <div className="mb-10 flex flex-col md:flex-row gap-4 items-stretch animate-in fade-in duration-300">
            ...
            <div className="flex bg-white dark:bg-slate-900 p-1.5 rounded-2xl shadow-xl shadow-sky-500/5 items-center overflow-x-auto no-scrollbar">
              <button
                ...
                className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${selectedRole === "All" ? "bg-sky-500 text-white shadow-lg" : "text-gray-400 hover:text-sky-500"}`}
              >
                Tous
              </button>
              {Object.values(Role).map((role) => (
                <button
                  ...
                  className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${selectedRole === role ? "bg-sky-500 text-white shadow-lg" : "text-gray-400 hover:text-sky-500"}`}
                >
                  {role}
                </button>
              ))}
        ...
        {viewMode === "offices" ? (
          <OfficesAvailabilityView
            offices={offices}
            projects={projects}
            onUpdateAssignment={handleGlobalAssignmentUpdate}
      ...
      {editingMember && (
        <EditMemberModal
          ...
          onSave={(u) => handleUpdateMember(u, editingMember.projectId)}
        />
      )}
    </div>
  );
